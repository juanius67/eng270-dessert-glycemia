...
#parameters:
    G = c_double(params["Gb_mg_dL"]) #converting to C double types for passing to the C function
    X = c_double(0.0)
    ...
    for idx, t in enumerate(times):
        #updating values based on C outputs:
        glucose[idx] = G.value
        insulin[idx] = I.value
        ...
        bindings.lib.step(byref(G), byref(X), byref(I), Gb.value, Ib.value, dt_c.value, D_eff, u_eff) #passing C double adresses/values to the C function
        # Note for myself: byref() passes the address of the variable in RAM, so that the C function can modify its value directly.
...
def _clip_series(times: np.ndarray, values: np.ndarray, t_limit: float) -> Tuple[np.ndarray, np.ndarray]:
    """Truncates time-series data to a specific limit, interpolating the final value.

    This function slices the input arrays up to `t_limit`. If `t_limit` falls between
    two existing time points, it performs linear interpolation to calculate the
    exact value at `t_limit` and appends this new point to the end. This ensures the
    series ends exactly at the specified time, which is critical for accurate AUC calculations.

    Args:
        times (np.ndarray): Monotonically increasing array of time points.
        values (np.ndarray): Array of corresponding values.
        t_limit (float): The time cutoff limit.

    Returns:
        Tuple[np.ndarray, np.ndarray]: A tuple containing the clipped time array
        (ending at t_limit) and the clipped value array.
...
#the following originally used SciPy for integration, but was retooled with the help of ChatGPT 5.0 to use NumPy only, as SciPy is a heavy dependency for a single function and caused bugs on my device#
...
####The following definition is a retooled function developped, initially, with the help of AI, as it explained the importance of helper functions and enclosed definitions###
def plot_result(result: SimulationResult, params: Dict[str, float], zoom_end: float, full_end: float, out_root: Path, dpi: int) -> None:
    ...
    def _plot(range_end: float, path: Path) -> None:
        ...
        ax_g.plot(times, glucose, label="Plasma Glucose $G(t)$ (mg/dL)", color="#1f77b4")
        ax_g.axhline(params["Gb_mg_dL"], linestyle="--", color="#444444", linewidth=1.0, label="Basal Glucose $G_b$")
        ax_g.set_ylabel("Glucose Concentration (mg/dL)", fontsize=12)
        ax_g.legend(loc="upper right", fontsize=12)
        ymin1, _ = ax_g.get_ylim()
        ymax2 = ax_g.get_ylim()[1]
        ax_g.set_ylim(ymin1, max(ymax2,220))

        ax_i.plot(times, insulin, label="Plasma Insulin $I(t)$ (μU/mL)", color="#d62728")
        ax_i.axhline(params["Ib_uU_mL"], linestyle="--", color="#555555", linewidth=1.0, label="Basal Insulin $I_b$")
        ax_i.set_ylabel("Insulin Concentration (μU/mL)", fontsize=12)
        ax_i.set_xlabel("Time post-meal (min)", fontsize=12)
        ...
        ax_i2 = ax_i.twinx()
        ax_i2.plot(times, D_total, color="#2ca02c", alpha=0.6, label="Glucose Appearance $D(t)$")
        ax_i2.plot(times, u_series, color="#9467bd", alpha=0.6, label="Insulin Stimulus $u(t)$")
        ax_i2.set_ylabel("Input Rates (D, u)", fontsize=12)
        lines, labels = ax_i.get_legend_handles_labels()
        lines2, labels2 = ax_i2.get_legend_handles_labels()
        ax_i.legend(lines + lines2, labels + labels2, loc="upper right", fontsize=12)
...
##############Code below enhanced (retooled to be more readable) with the help of ChatGPT 5.1#########
...
##############Code above enhanced with the help of ChatGPT 5.1#########
...
###END OF SECTION GENERATED BY GEMINI 3.0 FOR DEBUGGING PURPOSES###
    ...
    zoom_end = float(zoom_choice.split("-")[1]) #extracting the end time from the string
    full_end = 1440.0
    ...
    #GENERATING BAR CHART DASHBOARD USING MATPLOTLIB##
    if generate_plots and summary_rows:
        with summary_path.open() as f:
            data = list(csv.DictReader(f))
            
        parameters = [
            ("peak_delta_G_mg_dL", "Peak G (mg/dL)"),
            ("iAUC_0_240_mg_dL_min", "iAUC (0-240) mg/dL·min"),
            ("time_to_baseline_min", "Time to Base (min)"),
            ("t_peak_min", "Time to Peak (min)")]

        fig, axes = plt.subplots(1, 4, figsize=(12, 3), constrained_layout=True) 
        desserts = [row["dessert"] for row in data]
        colors = ["#1f77b4", "#ff7f0e", "#2ca02c", "#d62728"] 

        for ax, (key, label) in zip(axes, parameters): 
            values = [float(row[key]) for row in data]
            bars = ax.bar(desserts, values, color=colors)
            ax.set_title(label, fontsize=14, fontweight='bold')
            ax.set_xticks([]) 

        fig.legend(bars, desserts, loc="upper center", ncol=4, bbox_to_anchor=(0.5, -0.05), fontsize=16)

        fig.savefig(str(outdir / "figures" / "summary_dashboard.png"), dpi=args.dpi, bbox_inches='tight')
        plt.close(fig)
